...
bool IsOptimizerSanityInvalid()
{
   // Validate active ADX settings
   if(AdxMode == ADX_FILTER_MIN_MAX && AdxMin >= AdxMax) return true;

   // Only validate specific trailing inputs if that specific mode is active
   if(SelectedTrail == TRAIL_PSAR)
   {
   ...
   // Only validate Take Profit inputs if the corresponding mode is active
...
void InitializeHistoricalRange()
{
   ...
   for(int i = 0; i < copied - LookbackMinutes; i++)
   {
      ...
      {
         // Sample strictly completed bars prior to the open window bar (i)
...
//+------------------------------------------------------------------+
//| Trade routing execution directly with explicit TP criteria        |
//+------------------------------------------------------------------+
void ExecuteByMode(ENUM_ORDER_TYPE type, double volume, double sl)
{
...
bool EnableTrade() 
{
   return (GetPositionsCount() == 0 && IsInWindow() && openHigh != 0 && !IsHoliday(TimeCurrent()));
}
