...
namespace cAlgo.Robots
{
    ...
    public class RangeBreakout : Robot
    {
        ...
        [Parameter("Min ADX Slope (Velocity)", DefaultValue = 0.05, Group = "3a. Derivative Filters")]
        public double MinAdxSlope { get; set; }
        [Parameter("Filter RSI Deceleration", DefaultValue = true, Group = "3a. Derivative Filters")]
        public bool FilterRsiDeceleration { get; set; }
        ...
        public double GetSynchronizedAdx(MT5ADXEquivalent AdxIndicator, Bars sourceBars, int targetBarIndex)
        {
            ...
            int AdxIndex = AdxIndicator.Bars.OpenTimes.GetIndexByTime(barTime);
        ...
        // --- NEW DERIVATIVE CALCULATION HELPERS ---
        private double GetSlope(DataSeries series, int index)
        {
            if (index < 1) return 0;
            return series[index] - series[index - 1];
        }

        private double GetAcceleration(DataSeries series, int index)
        {
            if (index < 2) return 0;
            double currentSlope = series[index] - series[index - 1];
            double previousSlope = series[index - 1] - series[index - 2];
            return currentSlope - previousSlope;
        }

        private double GetCustomAdxSlope(MT5ADXEquivalent adxIndicator, Bars sourceBars, int targetBarIndex)
        {
            double currentAdx = GetSynchronizedAdx(adxIndicator, sourceBars, targetBarIndex);
            double prevAdx = GetSynchronizedAdx(adxIndicator, sourceBars, targetBarIndex - 1);
            
            if (double.IsNaN(currentAdx) || double.IsNaN(prevAdx)) return 0;
        ...
        protected override void OnBar()
        {
            ...
            if (AdxMode != AdxFilterMode.Off)
            {
                ...
                {
                    Print("Waiting for Adx indicator history to warm up safely...");
                    return;
                }
            }
            ...
            if (EnableTrade() && Positions.Count(p => p.Label == Label) < MaxPositions)
            {
                ...
                if(!m5AdxPass) Print("FILTER_REJECT: m5AdxPass failed.");
                if(!htfAdxPass) Print("FILTER_REJECT: htfAdxPass failed");
                
                if (AdxMode != AdxFilterMode.Off)
                {
                    Print("True Adx = ", GetSynchronizedAdx(_customAdx, _execBars, _execBars.Count - 2), 
                          ". True HTF Adx = ", GetSynchronizedAdx(_customHtfAdx, _execBars, _execBars.Count - 2));
                }
        ...
        private void TradeIfAble()
        {
            ...
            // --- EVALUATE NEW ADX SLOPE VELOCITY GATE ---
            if (AdxMode != AdxFilterMode.Off)
            {
                double currentHtfSlope = GetCustomAdxSlope(_customHtfAdx, _execBars, _execBars.Count - 2);
                if (currentHtfSlope < MinAdxSlope)
                {
                    Print("FILTER_REJECT: HTF ADX Velocity too sluggish ({0} < {1})", currentHtfSlope, MinAdxSlope);
                    return;
                }
            }

            // --- EVALUATE NEW RSI ACCELERATION TRAP GATE ---
            if (FilterRsiDeceleration)
            {
                double rsiAcceleration = GetAcceleration(_rsi.Result, _execBars.Count - 2);
                
                // If breaking out long, but momentum expansion is rolling over/decelerating
                if (signalBar.Close > openHigh && rsiVal > RSIVal && rsiAcceleration < 0)
                {
                    Print("FILTER_REJECT: Bullish breakout canceled. RSI is decelerating ({0})", rsiAcceleration);
                    return;
                }
                // If breaking out short, but downward momentum expansion is flattening out
                if (signalBar.Close < openLow && rsiVal < (100 - RSIVal) && rsiAcceleration > 0)
                {
                    Print("FILTER_REJECT: Bearish breakout canceled. RSI downward acceleration flattening ({0})", rsiAcceleration);
                    return;
                }
            }
            ...
            if ((bullSig && (!signalTouchZone && !gapUpBreakout)) || (bearSig && (!signalTouchZone && !gapDownBreakout))) Print("FILTER_REJECT: No breakout detected");
        }
